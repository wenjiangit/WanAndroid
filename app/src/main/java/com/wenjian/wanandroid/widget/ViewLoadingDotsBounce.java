...
public class ViewLoadingDotsBounce extends LinearLayout {
    ...
    private static final int[] ATTR_RES = {android.R.attr.radius};
    ...
    public ViewLoadingDotsBounce(Context context, @Nullable AttributeSet attrs) {
        ...
        TypedArray typedArray = context.obtainStyledAttributes(attrs, ATTR_RES);
        try {
            radius = typedArray.getDimensionPixelSize(0, 100);
        } finally {
            typedArray.recycle();
        }
    ...
    private void animateView() {
        ...
        for (int i = 0; i < OBJECT_SIZE; i++) {
            ...
            PropertyValuesHolder y = PropertyValuesHolder.ofFloat(View.TRANSLATION_Y, -getHeight() / POST_DIV);
            PropertyValuesHolder x = PropertyValuesHolder.ofFloat(View.TRANSLATION_X, 0);
